...
<div class="am-content">
    ...
    <div class="am-card" style="margin-bottom:16px;">
        <div class="am-card__toolbar">
            <form method="GET" action="{{ url('/process_audit') }}" class="am-search" id="amPaSearchForm" style="flex:1;max-width:340px;margin:0;">
                <i class="fa fa-search"></i>
                <input type="text" name="q" id="amPaSearch" value="{{ $search ?? '' }}" placeholder="Search audits…" autocomplete="off">
            </form>
            <a href="{{ asset('download_process_audit/Process-Audit.pdf') }}" target="_blank" class="am-btn am-btn-outline">
    ...
    <div class="am-card">
        <div id="amPaContainer">
            @include('dashboard.form_records.partials.process_audit_table')
        </div>
    </div>
...
<div class="am-modal" id="editPaModal" role="dialog" aria-modal="true">
    <div class="am-modal__box am-form" style="max-width:1000px;">
        ...
        <form action="{{ route('auditDelete') }}" method="POST" enctype="multipart/form-data" style="display:contents;">
            @csrf
...
<script>
    ...
    function amPaEdit(d) {
        document.getElementById('epa-id').value = d.id || '';
        ['auditor', 'auditDate', 'nonConformities', 'Observations', 'nonConfReport', 'dateFrequency', 'AdutiActions', 'any_issues'].forEach(function(k) {
            var el = document.querySelector("#editPaModal input[name='" + k + "'], #editPaModal textarea[name='" + k + "']");
            if (el) el.value = d[k] || '';
        });
        var sel = document.querySelector("#editPaModal select[name='processAudit']");
        if (sel) sel.value = d.processAudit || '';
        paChecklistFields.forEach(function(f) {
            document.querySelectorAll("#editPaModal input[name='" + f[0] + "']").forEach(function(r) { r.checked = false; });
            if (d[f[0]]) {
                var r = document.querySelector("#editPaModal input[name='" + f[0] + "'][value='" + d[f[0]] + "']");
                if (r) r.checked = true;
            }
            if (f[2]) {
                var ev = document.querySelector("#editPaModal input[name='" + f[2] + "']");
                if (ev) ev.value = d[f[2]] || '';
            }
        });
    ...
    // ---- Download PDF ----
    function downloadAuditPdf(btn) {
        var processId = btn.getAttribute('data-processid');
        ...
        fetch('/generate-pdf', {
            ...
            body: 'process_id=' + encodeURIComponent(processId)
        })
        .then(function(r) { return r.json(); })
        .then(function(data) { if (data.url) window.open(data.url, '_blank'); })
        .catch(function() { alert('Failed to generate PDF.'); });
    }
