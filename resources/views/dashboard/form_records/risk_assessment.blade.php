    ...
    <div class="am-modal__box am-form" style="max-width:900px;">
        ...
        <form action="{{ route('editassessment') }}" method="POST" style="display:contents;">
            ...
            <div class="am-modal__body" style="padding:20px;">
                ...
                @php
                    $radios = [
                        'qualitySatandard'  => ['Meet Quality Standard?',    ['Yes','No','NA'], 'commentsstandard'],
                        'delevryStandard'   => ['Meet Delivery Standard?',   ['yes','no','NA'], 'commentsdelvery'],
                        'priceRequiremnt'   => ['Meet Price?',               ['yes','No','NA'], 'commentprice'],
                        'interestedDeemed'  => ['Interested Parties?',       ['Yes','No','NA'], 'commentsDeemed'],
                    ];
                @endphp
                @foreach($radios as $name => [$label, $opts, $commentField])
                <div class="form-group row">
                    <div class="col-lg-6">
                        <label>{{ $label }}</label>
                    ...
                    <div class="col-lg-6"><label>Comments</label><input type="text" class="form-control" name="{{ $commentField }}"></div>
                </div>
                @endforeach
                <div class="form-group row">
...
(function() {
    var t = document.getElementById('toggleRaForm');
    var f = document.getElementById('newRaForm');
    var c = document.getElementById('cancelRaForm');
...
(function() {
    ...
    if (!container) return;
    var baseUrl = '{{ url('/risk_assessment') }}';
...
function amRaEdit(d) {
    document.getElementById('era-id').value = d.id || '';
    var m = document.getElementById('editModal');
    ['jobNumber','date','dateDevelry','DecisionComment','commentprice','commentsDeemed','commentsdelvery','commentsstandard'].forEach(function(k){
        var el = m.querySelector("input[name='"+k+"']");
        if (el) el.value = d[k] || '';
    });
    ['qualitySatandard','delevryStandard','priceRequiremnt','interestedDeemed'].forEach(function(k){
        m.querySelectorAll("input[name='"+k+"']").forEach(function(r){ r.checked = false; });
        var target = m.querySelector("input[name='"+k+"'][value='"+d[k]+"']");
        if (target) target.checked = true;
    });
    var sel1 = m.querySelector("select[name='RiskProbability']"); if (sel1) sel1.value = d.RiskProbability || '';
    var sel2 = m.querySelector("select[name='riskSeverity']"); if (sel2) sel2.value = d.riskSeverity || '';
