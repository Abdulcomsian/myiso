    ...
    <table class="am-table" id="amReqTable">
        ...
        <tbody>
            @forelse ($getReq as $index => $data)
                ...
                @endphp
                <tr>
                    <td><span class="am-cell-sub">#{{ $getReq->firstItem() + $index }}</span></td>
                    <td>
...
<div class="am-pagination">
    <div class="am-pagination__info">
        Showing <strong>{{ $getReq->firstItem() ?? 0 }}–{{ $getReq->lastItem() ?? 0 }}</strong> of <strong>{{ $getReq->total() }}</strong>
    </div>
    <div class="am-pagination__nav">
        @if ($getReq->onFirstPage())
            <button disabled>‹</button>
        @else
            <button data-page="{{ $getReq->currentPage() - 1 }}" class="am-page-link">‹</button>
        @endif

        @php
            $current = $getReq->currentPage();
            $last    = $getReq->lastPage();
