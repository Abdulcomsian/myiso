    ...
    <table class="am-table" id="amPaTable">
        ...
        <tbody>
            @forelse ($getprocess as $index => $data)
                ...
                <tr>
                    <td><span class="am-cell-sub">#{{ $getprocess->firstItem() + $index }}</span></td>
...
<div class="am-pagination">
    <div class="am-pagination__info">
        Showing <strong>{{ $getprocess->firstItem() ?? 0 }}–{{ $getprocess->lastItem() ?? 0 }}</strong> of <strong>{{ $getprocess->total() }}</strong>
    </div>
    <div class="am-pagination__nav">
        @if ($getprocess->onFirstPage())
            <button disabled>‹</button>
        @else
            <button data-page="{{ $getprocess->currentPage() - 1 }}" class="am-page-link">‹</button>
        @endif

        @php
            $current = $getprocess->currentPage();
            $last    = $getprocess->lastPage();
