        ...
        <tbody>
            @forelse ($auditreport as $index => $item)
                <tr>
                    ...
                    <td>
                        <span class="am-cell-primary">QMS Audit #{{ $item->QmsauditNumber ?? ($auditreport->firstItem() + $index) }}</span>
...
<div class="am-pagination">
    <div class="am-pagination__info">
        Showing <strong>{{ $auditreport->firstItem() ?? 0 }}–{{ $auditreport->lastItem() ?? 0 }}</strong> of <strong>{{ $auditreport->total() }}</strong>
    </div>
